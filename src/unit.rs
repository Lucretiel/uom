/// Macro to implement a set of [measurement units][measurement]. Note that this macro must be
/// executed in direct submodules of the module where the [`system!`](macro.system.html) macro was
...
/// The `#[macro_use]` attribute must be used when including the `uom` crate to make the
...
macro_rules! unit {
    (
        ...
        unit!($($(#[$unit_attr])* @$unit: $($conversion),+; $abbreviation, $singular, $plural;)+);
    };
    (
        $($(#[$unit_attr:meta])* @$unit:ident: $($conversion:expr),+;
            $abbreviation:expr, $singular:expr, $plural:expr;)+
    ) => {
        $(unit!(@unit $(#[$unit_attr])* @$unit);
